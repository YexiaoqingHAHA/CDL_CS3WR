    ...
    for n_topic in range(5, max_hidden_neuron+1, 5):
        ...
        for test_fold in range(1, test_folds + 1):
            if data_name == "citeulike-a":
                ...
                results_path_data = results_path + 'citeulike-a/variable/'

            elif data_name == "citeulike-t":
                num_user = 7947
                num_item = 25975
                test_file = data_path + 'cf-t/' + 'cf-t-test-' + str(test_fold) + '-users.dat'
                results_path_data = results_path + 'citeulike-t/variable/'
            ...
            u_file = results_path_sub + 'U.txt'
            v_file = results_path_sub + 'V.txt'
...
def integrate_decision_and_teaching_cost(max_hidden_neuron,test_folds,data_name,first_hidden_nueron,data_path,results_path,cost_dict,theta,thre_rating,a,b):
    ...
    for n_topic in range(5, max_hidden_neuron+1, 5):
        for test_fold in range(1,test_folds+1):
            ...
            V = np.mat(np.loadtxt(v_file, dtype=np.float))
            estimated_R = U * V.T
            estimated_R = estimated_R.clip(0, 1)
...
if __name__=="__main__":
    ...
    for data_name in data_list:
     #   integrate_precision_and_recall(max_hidden_neuron, test_folds, data_name, data_path, results_path)
        integrate_decision_and_teaching_cost(max_hidden_neuron,test_folds,data_name,first_hidden_nueron,data_path,results_path,cost_dict,theta,thre_rating,a,b)
