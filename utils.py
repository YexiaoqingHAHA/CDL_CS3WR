...
def variable_save(result_path,model_name,train_var_list1,train_var_list2,U,V,Encoded_X,test_v_ud,mask_test_v_ud,hidden_neuron,test_fold):
    result_path=result_path+'variable/'+str(hidden_neuron)+"-"+str(test_fold)+"/"
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    time.sleep(5)
    '''
    ...
    for var in train_var_list2:
        if model_name == "DIPEN_with_VAE":
            ...
            np.savetxt(result_path +str(hidden_neuron)+str(test_fold)+"-"+"-"+var_name , var_value)
    '''
    np.savetxt(result_path+"U.txt",U)
    np.savetxt(result_path+"V.txt",V)
    np.savetxt(result_path+"Topic.txt",Encoded_X)
