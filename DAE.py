# -*- coding: utf-8 -*-
"""
Created on Sat Sep  1 20:55:43 2018
...
class DAE():
    def __init__(self,n_visible,n_hidden,num_item,num_voca,itr,dipen_activation,lambda_w=33.3333):
    ...
    def do_pretrain(self,pretrain_input,epoch,batch_size,learning_rate,dropout,corruption_level):
        ...
        if self.dipen_activation=="sigmoid":
            ...
            tmp_const=1
        '''    
        pre_W1=tf.get_variable(name=("pre_W1"+str(self.itr)),shape=[self.n_visible,self.n_hidden],
                               dtype=tf.float32,initializer=tf.random_normal_initializer())
        pre_b1=tf.get_variable(name=("pre_b1"+str(self.itr)),shape=[self.n_hidden],dtype=tf.float32,
                               initializer=tf.random_normal_initilizer())
        pre_W2=tf.get_variable(name=("pre_W2"+str(self.itr)),shape=[self.n_hidden,self.n_visible],
                               dtype=tf.float32,initializer=tf.random_normal_initializer())
        ...
        mask=tf.placeholder(tf.float32,[None,self.n_visible],name="mask")
        mask_np=np.random.binomial(1,1-corruption_level,(self.num_item,self.n_visible))
        corrupted_X=tf.multiply(mask,X)
        ...
        # Define loss and optimizder,minimize the squared error
        ...
        with tf.Session() as sess:
            ...
            random_per_doc_idx=list(np.random.permutation(self.num_item))
            # Training cycle
            for epoch in range(epoch):
                ...
                for i in range(total_batch+1):
                    if i< total_batch:
                        batch_set_idx=random_per_doc_idx[i*batch_size:(i+1)*batch_size]
                    elif i==total_batch:
                        batch_set_idx=random_per_doc_idx[i*batch_size:]
            ...
            next_pretrain_input=tf.nn.sigmoid(tf.add(tf.matmul(pretrain_input,pre_W1),pre_b1))
